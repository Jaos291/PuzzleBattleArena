...
public class GridManager : MonoBehaviour
{
    ...
    public int maxVisibleRows = 10;  // N�mero m�ximo de filas visibles en pantalla
    ...
    void Update()
    {
        ...
        if (!IsGridAtTop())
        {
            ...
            // Si el offset llega a 1, entonces hemos subido una unidad completa y es momento de a�adir una nueva l�nea
            if (gridOffset >= 1f)
            {
                gridOffset = 0f;
                AddNewLine();
            ...
            // Actualizar la posici�n de los bloques
    ...
    bool IsGridAtTop()
    {
        for (int x = 0; x < gridWidth; x++)
        {
            ...
            {
                return true; // Si alguna celda en la fila superior est� ocupada, llegamos al tope
            }
        }
        return false;
    }

    // Actualiza la posici�n de todos los bloques seg�n el offset del grid
        ...
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                if (gridArray[x, y] != null)
                {
                    ...
                    // Si el bloque est� en la fila 1 o m�s arriba, hacerlo usable
    ...
    void SwapBlocks(Vector2Int block1Pos, Vector2Int block2Pos)
    {
        if (IsValidPosition(block1Pos) && IsValidPosition(block2Pos))
        {
            ...
            if (block1 != null && block2 == null) // Si el cubo destino est� vac�o
            {
                ...
                // Animar el movimiento horizontal hacia la nueva posici�n
                StartCoroutine(SmoothMove(block1, new Vector3(block2Pos.x, block2Pos.y + gridOffset, 0), 0.15f, () => {
                    // Despu�s de moverse horizontalmente, verificamos si debe caer
    ...
    IEnumerator SmoothMove(GameObject block, Vector3 targetPosition, float duration, System.Action onComplete = null)
    {
        ...
        // Llamar a la funci�n de callback si se proporciona
        onComplete?.Invoke();
    }
    ...
    IEnumerator CheckMatchesAndDestroy(Vector2Int pos1, Vector2Int pos2)
    {
        ...
        //StartCoroutine(HandleBlockFall());  // Hacer que los bloques caigan si hay espacio vac�o debajo
    }
    ...
    public bool CheckMatches(Vector2Int pos)
    {
        ...
        // Si hay 3 o m�s coincidencias horizontales o verticales, marcar los bloques para desaparecer
